...
unsigned int SidTune::selectSong(const unsigned int selectedSong)
{
    ...
    {
        song = info->m_startSong;
        m_statusString = txt_songNumberExceed;
    }
...
bool SidTune::loadFile(const char* fileName, Buffer_sidtt<const uint_least8_t>& bufferRef)
{
    ...
    if ( !myIn.is_open() )
    {
        m_statusString = txt_cantOpenFile;
        return false;
    }
...
#endif
    {
        m_statusString = txt_notEnoughMemory;
        return false;
    }
    ...
    if ( myIn.bad() )
    {
        m_statusString = txt_cantLoadFile;
        return false;
    }
...
void SidTune::getFromBuffer(const uint_least8_t* const buffer, const uint_least32_t bufferLen)
{
    ...
    {
        m_statusString = txt_fileTooLong;
        return;
    }
    ...
    try
    {
        ...
        else
        {
            ...
            else
            {
                ...
                m_statusString = txt_unrecognizedFormat;
            }
        }
    }
    catch (loadError& e)
                            ...
                            Buffer_sidtt<const uint_least8_t>& buf)
{
    ...
    {
        m_statusString = txt_dataTooLong;
        return false;
    }
...
bool SidTune::saveC64dataFile( const char* fileName, bool overWriteFlag )
{
    ...
    if ( status )
    {
        ...
        {
            m_statusString = txt_cantCreateFile;
        }
        else
        {
            if ( !info->m_musPlayer )
