...
{
    int offset;
    if (nmiFlag && cycleCount > nmiClk + 2) {
        nmiFlag = false;
        offset = oNMI;
    } else if (!flagI && irqFlag && cycleCount > irqClk + 2) {
        offset = oIRQ;
    } else {
#ifdef MOS6510_DEBUG
        if (dodump)
...
{
    Cycle_EffectiveAddress = env->envReadMemDataByte (Cycle_Pointer);
}
...
{   // Rev 1.03 (Mike) - Extra +1 removed
    ...
    endian_16hi8 (Cycle_EffectiveAddress, env->envReadMemDataByte (Cycle_Pointer));
}
 ...
 m_steal("CPU-steal", *this, &MOS6510::eventWithSteals)
{
    ...
    for (int i = 0; i < 0x100; i++)
    {
        ...
        */
        ...
        struct ProcessorCycle *instrCurrent = instrTable[i];
        int cycleCount = 0;
...
*/
        ...
        case PLPn:
            ...
            instrCurrent[cycleCount++].func = &MOS6510::plp_instr;
        break;
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        */
        ...
        case oRST:
            ...
            instrCurrent[cycleCount++].func = &MOS6510::RSTHiRequest;
        break;
...
{   // Internal Stuff
    ...
    endian_16lo8 (Cycle_EffectiveAddress, env->envReadMemDataByte (0xFFFC));
    endian_16hi8 (Cycle_EffectiveAddress, env->envReadMemDataByte (0xFFFD));
    Register_ProgramCounter = Cycle_EffectiveAddress;
//    filepos = 0;
}
