...
[
    ...
    [AS_HELP_STRING([--disable-library-checks],
        [do not check for working libraries])])
...
[
    if test "$SID2_LIB_CHECK" != "0"; then
        MY_FIND_PKG_CONFIG_LIB(sidplay2,["2.1.0"],
            [builders $1],[sidplay/sidplay2.h],[sidplay2 *myEngine])
    else
