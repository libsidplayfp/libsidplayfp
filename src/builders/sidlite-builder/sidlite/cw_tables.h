...
//This simplified schematic example might make it easier to understand sawtooth+pulse combination (must be observed with monospace fonts):
//                               _____            |-    .--------------.   /\/\--.
...
using cw_array_t = std::array<unsigned char, Length>;
...
#  include <stdexcept>
    ...
    constexpr float cpow(float base, int exp)
    {
        if (exp<0) throw std::logic_error("exp must be positive");
        return exp == 0 ? 1.f : base * cpow(base, exp-1);
    }

    constexpr auto createCombinedWF(float bitmul, float bitstrength, float treshold)
    {
        cw_array_t WFarray {};
